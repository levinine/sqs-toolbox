...
const apiFunctions = (sqs) => {
        ...
        return await Promise.all([listQueues(source), listQueues(target)]).then(
            (response) => {
                ...
                if (typeof response[0] === 'undefined') {
                    // add source queue does not exist print
                    process.exit();
                }
                ...
                if (typeof target === 'undefined') {
                    ...
                    if (typeof response[1] === 'undefined') {
                        // add target queue does not exist print
                        process.exit();
                    }
    ...
    const checkIfQueueExists = async (queueName) => {
        const queueList = await listQueues();
        const found = queueList.find(queue => queue.name === queueName);
    ...
    const purgeQueue = async (QueueName) => {
        ...
        const getQueueUrlResponse = await getQueueUrl(QueueName);
        if (getQueueUrlResponse) QueueUrl = getQueueUrlResponse['QueueUrl'];
        return await sqs.purgeQueue({ QueueUrl }).promise();
    };
